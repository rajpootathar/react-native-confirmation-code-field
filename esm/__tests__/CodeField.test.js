...
const renderCodeField = (props) => render(<CodeField {...requiredProps} {...props}/>);
it('should use custom root component', () => {
    const id = 'custom-root';
    const RootComponent = (p) => (<View {...p} testID={id}/>);
...
{
    console.log(<CodeField renderCell={() => null} ref={(ref) => {
            ref?.focus();
            ref?.blur();
        }}/>);
    console.log(
    // @ts-expect-error - number is not function
    <CodeField onChangeText={123} renderCell={() => null} ref={(ref) => {
            ref?.focus();
            ref?.blur();
        }}/>);
}
{
    ...
    const MyTextInput = forwardRef((props, ref) => (<View testID={props.myRequiredProps}>
        <TextInput ref={ref}/>
      </View>));
    console.log(<CodeField renderCell={() => null} myRequiredProps="my-required-props" InputComponent={MyTextInput}/>);
    console.log(
    ...
    // @ts-expect-error - `myRequiredProps` is required prop
    <CodeField renderCell={() => null} InputComponent={MyTextInput}/>);
    console.log(<CodeField renderCell={() => null} 
